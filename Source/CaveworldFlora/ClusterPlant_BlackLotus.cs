...
namespace CaveworldFlora
{
    ...
    public class ClusterPlant_BlackLotus : ClusterPlant
    {
                        ...
                        if (num != 0f)
                        {
                                ...
                                && pawn.IsColonist)
                            {
                                Find.LetterStack.ReceiveLetter("CaveworldFlora.LetterLabelBlackLotus".Translate(), "CaveworldFlora.BlackLotus".Translate(),
                                    LetterDefOf.NegativeEvent, new RimWorld.Planet.GlobalTargetInfo(pawn));
        ...
        {
            Vector3 spawnPosition = Position.ToVector3Shifted() + Vector3Utility.RandomHorizontalOffset(3f);
